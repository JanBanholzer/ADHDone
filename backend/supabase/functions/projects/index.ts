  ...
  try {
    switch (req.method) {
      case "GET": {
        ...
        if (id) {
          const { data, error: e } = await db
            .from("projects")
            .select(
              "*, missions(id, title), quests(id, title, status, target_date, sort_order)",
            )
            .eq("id", id)
            .single();
        ...
        let query = db.from("projects").select("*, missions(id, title)");
      ...
      case "POST": {
        ...
        if (!body.mission_id) return error("mission_id is required");
